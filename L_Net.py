...
class L_Net(ResNet34):
    def __init__(self, num_classes):
        ...
        for i in range(num_classes):
            self.lstm = nn.LSTM(512, 49, 1, dropout=0.2).to(self.device)
            self.lstm_list.append(self.lstm)

    def init(self,x):
        h0 = torch.empty(1, x.shape[0], 49).to(self.device)
        c0 = torch.empty(1, x.shape[0], 49).to(self.device)
    ...
    def forward(self, x):  # 224x224x3
        ...
        out=torch.zeros([input.shape[1],12]).to(self.device)   #LSTM
        for i in range(12):
